...
public:
    ...
    virtual T operator ()(const request_line& ln) const {
        if(req_func) return req_func(ln);
        return T();
    }

    virtual T operator ()(const response_line& ln) const {
        if(resp_func) return resp_func(ln);
        return T();
    }
};
...
struct http_field{
    ...
    std::string getParam(std::string name) {
        if(params.count(name)) return params.at(name);
        return std::string();
    }
};
BOOST_FUSION_ADAPT_STRUCT(
...
public:
    ...
    http_raw_packet parse(std::string raw) {
        ...
        start_line_parser p;
        auto uri_with_params = p(pack.start).get<request_line>().uri;
        pack.query_params = parse_query_params(uri_with_params);

        std::vector<std::string> splited;
        boost::split(splited, uri_with_params, boost::is_any_of("?"));
        pack.route = splited[0];
        return pack;
    }
...
public:
    ...
    http_field operator[](std::string name) const {
        if(origin->body.count(name)) return origin->body.at(name);
        return http_field();
    }
